...
class CandlingDataCollector:
    def __init__(self, root):
        self.root = root
        self.root.title("🥚 Duck Egg Candling Center (YOLOv8 Live)")
        ...
        try:
            ...
            messagebox.showerror("Model Error", f"Could not load YOLOv8 model from 'weights/best.pt'.\nError: {e}")
            self.root.destroy()
            return
        ...
        # Background framing + AI inference loop
    ...
    def setup_ui(self):
        ...
        meta_frame = tk.LabelFrame(self.sidebar, text=" 4. Annotation & Metadata (For Manual Save) ", font=("Arial", 10, "bold"), bg="#f5f6fa", fg="#2f3640", padx=10, pady=10)
    ...
    def save_image(self):
        with self.frame_lock:
            # We intentionally save the raw frame here so your saved dataset images 
            # stay completely clean and free of bounding box overlays!
            frame_to_save = self.latest_raw_frame.copy() if self.latest_raw_frame is not None else None
        ...
        if frame_to_save is not None:
            ...
            self.snap_btn.config(bg="#2ed573", fg="white", text="✔ RAW SAMPLE RECORDED")
    ...
    def _clear_visual_flash(self):
        ...
        self.snap_btn.config(bg="#2cf43b", fg="#1e272e", text="📸 CAPTURE CLEAN SAMPLE\n[ Spacebar Key ]")
